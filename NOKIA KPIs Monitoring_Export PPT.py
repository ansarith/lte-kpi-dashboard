...
def aggregate_data(df, kpis, daily=False, group=False):
    ...
    agg_dict = {}
    for kpi in kpis:
        if kpi in ["PDCP SDU Volume, DL","PDCP SDU Volume, UL","Total LTE data volume, DL + UL","Avg RRC conn UE","RRC Connected UEs Max (M8051C56)"]:
            agg_dict[kpi] = "sum"
        else:
            agg_dict[kpi] = "mean"
    ...
    for idx, selected_kpi in enumerate(selected_kpis[:4]):
        ...
        if not group_option and "LNCEL name" in plot_df.columns:
            for i, cell in enumerate(plot_df["LNCEL name"].unique()):
                ...
                fig.add_trace(go.Scatter(x=cell_df["Time_str"], y=cell_df[selected_kpi], mode="lines+markers", name=cell, line=dict(color=colors[i % len(colors)])))
        else:
            fig.add_trace(go.Scatter(x=plot_df["Time_str"], y=plot_df[selected_kpi], mode="lines+markers", name=selected_kpi))
        fig.update_layout(height=420, title=dict(text=selected_kpi, x=0.5), hovermode="x unified", margin=dict(l=40, r=120, t=60, b=40))
        cols[idx % 2].plotly_chart(fig, use_container_width=True)
        figures.append(fig)

# --- Export to PPT using matplotlib/PIL ---
def create_ppt(figures):
    prs = Presentation()
    prs.slide_width = Inches(13.33)
    prs.slide_height = Inches(7.5)
    positions = [(Inches(0.5), Inches(0.5)), (Inches(6.9), Inches(0.5)), (Inches(0.5), Inches(4.0)), (Inches(6.9), Inches(4.0))]
    chart_width = Inches(6.08)
    chart_height = Inches(3.04)

    for idx, fig in enumerate(figures):
        slide = prs.slides.add_slide(prs.slide_layouts[5]) if idx % 4 == 0 else slide
        # Save Plotly fig to PNG via PIL/matplotlib
        img_bytes = fig.to_image(format="png", width=900, height=450, scale=1)
        img_buf = io.BytesIO(img_bytes)
        slide.shapes.add_picture(img_buf, positions[idx % 4][0], positions[idx % 4][1], width=chart_width, height=chart_height)
...
if figures:
    ...
    st.download_button("📊 Download PowerPoint Report", data=ppt_file, file_name="LTE_KPI_Report.pptx", mime="application/vnd.openxmlformats-officedocument.presentationml.presentation")
else:
