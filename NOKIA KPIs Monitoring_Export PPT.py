import os, io
import streamlit as st
import pandas as pd
import plotly.graph_objects as go
import plotly.express as px
from pptx import Presentation
from pptx.util import Inches

st.set_page_config(layout="wide")
st.title("📊 LTE KPI Dashboard")

# ----------------------------
# Load Data
# ----------------------------
@st.cache_data
def load_data(path):
    df = pd.read_excel(path)
    df["Period start time"] = pd.to_datetime(df["Period start time"], errors="coerce")
    # Convert percentages to 0-100 scale if needed
    percentage_kpis = [col for col in df.columns if "%" in col or "Rate" in col]
    for col in percentage_kpis:
        if col in df.columns and pd.api.types.is_numeric_dtype(df[col]):
            if df[col].max() <= 1.0:
                df[col] = df[col] * 100
    return df

DATA_PATH = "4G_Main_KPIs_Report_SRAN21B-Sarith-2025_10_13-Site KCH2567RBR & 2070_LB.xlsx"
df = load_data(DATA_PATH)

# ----------------------------
# Select KPIs, Sites, Cells
# ----------------------------
kpi_columns = [col for col in df.columns if col not in ["Period start time", "LNBTS name", "LNCEL name"]]
selected_kpis = st.multiselect("Select KPI(s)", options=kpi_columns, default=kpi_columns[:4])

enodeb_selected = st.multiselect("Select LNBTS name", options=sorted(df["LNBTS name"].unique()))
cell_options = sorted(df[df["LNBTS name"].isin(enodeb_selected)]["LNCEL name"].unique()) if enodeb_selected else sorted(df["LNCEL name"].unique())
cell_selected = st.multiselect("Select LNCEL name", options=cell_options)

daily_option = st.checkbox("📅 Daily Aggregation")
group_option = st.checkbox("🏙️ Group by Site")

plot_df = df.copy()
if enodeb_selected:
    plot_df = plot_df[plot_df["LNBTS name"].isin(enodeb_selected)]
if cell_selected:
    plot_df = plot_df[plot_df["LNCEL name"].isin(cell_selected)]

# ----------------------------
# Aggregate Data
# ----------------------------
def aggregate_data(df, kpis, daily=False, group=False):
    for kpi in kpis:
        df[kpi] = pd.to_numeric(df[kpi], errors="coerce")

    agg_dict = {}
    sum_kpis = ["PDCP SDU Volume, DL", "PDCP SDU Volume, UL", "Total LTE data volume, DL + UL",
                "Avg RRC conn UE", "RRC Connected UEs Max (M8051C56)"]
    for kpi in kpis:
        agg_dict[kpi] = "sum" if kpi in sum_kpis else "mean"

    time_col = "Date" if daily else "Period start time"
    if daily:
        df["Date"] = df["Period start time"].dt.normalize()

    group_cols = [time_col] + ([] if group else ["LNCEL name"])
    return df.groupby(group_cols, as_index=False).agg(agg_dict)

plot_df = aggregate_data(plot_df, selected_kpis, daily_option, group_option)
time_col = "Date" if daily_option else "Period start time"
plot_df = plot_df.dropna(subset=[time_col])
plot_df["Time_str"] = plot_df[time_col].dt.strftime("%Y-%m-%d" if daily_option else "%Y-%m-%d %H:%M")

# ----------------------------
# Display Charts
# ----------------------------
figures = []
cols = st.columns(2)

for idx, kpi in enumerate(selected_kpis[:4]):
    fig = go.Figure()
    if not group_option:
        for i, cell in enumerate(plot_df["LNCEL name"].unique()):
            cell_df = plot_df[plot_df["LNCEL name"] == cell]
            fig.add_trace(go.Scatter(x=cell_df["Time_str"], y=cell_df[kpi],
                                     mode="lines+markers", name=cell))
    else:
        fig.add_trace(go.Scatter(x=plot_df["Time_str"], y=plot_df[kpi], mode="lines+markers", name=kpi))

    fig.update_layout(height=420, title=dict(text=kpi, x=0.5), hovermode="x unified", legend=dict(x=1.02, y=1))
    cols[idx % 2].plotly_chart(fig, use_container_width=True)
    figures.append(fig)

# ----------------------------
# Export PPT
# ----------------------------
def create_ppt(figures):
    prs = Presentation()
    prs.slide_width = Inches(13.33)
    prs.slide_height = Inches(7.5)

    positions = [(Inches(0.5), Inches(0.5)), (Inches(6.9), Inches(0.5)),
                 (Inches(0.5), Inches(4.0)), (Inches(6.9), Inches(4.0))]
    chart_width = Inches(6.08)
    chart_height = Inches(3.04)

    for idx, fig in enumerate(figures):
        slide = prs.slides.add_slide(prs.slide_layouts[5]) if idx % 4 == 0 else slide
        img_buf = io.BytesIO()
        try:
            img_bytes = fig.to_image(format="png", engine="kaleido")
            img_buf.write(img_bytes)
            img_buf.seek(0)
        except Exception:
            # Fallback: blank placeholder image
            import PIL.Image as Image
            blank = Image.new("RGB", (900, 450), color="white")
            blank.save(img_buf, format="PNG")
            img_buf.seek(0)
        pos_idx = idx % 4
        slide.shapes.add_picture(img_buf, positions[pos_idx][0], positions[pos_idx][1], width=chart_width, height=chart_height)

    ppt_buffer = io.BytesIO()
    prs.save(ppt_buffer)
    ppt_buffer.seek(0)
    return ppt_buffer

if figures:
    ppt_file = create_ppt(figures)
    st.download_button("📊 Download PowerPoint Report", ppt_file, "LTE_KPI_Report.pptx",
                       "application/vnd.openxmlformats-officedocument.presentationml.presentation")
